...
def draw_hand_skeleton(frame, results):
    mp_drawing.draw_landmarks(
        ...
        mp_drawing.DrawingSpec(color=BLACK_COLOR, thickness=2, circle_radius=2),
...
while cap.isOpened():
    ...
    _, gesture = gesture_detector.recognize(frame_copy)
    ...
    if holistic_results.left_hand_landmarks:
        ...
        if person_id is not None and gesture == "Follow":
            if follow_time is None:
                follow_time = time.time()
            elif time.time() - follow_time >= 2:
                tracking_id = person_id
                stop_time = None
        elif person_id is not None and gesture == "Stop":
